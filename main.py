...
bot_adi = "Scropion_game_azbot"

if hex(getnode()) in ["0xdc7b23bb434e"]: #windows masaüstü pc ise veya laptop
    #kaç yaşındasın bot
    print("kyb")
    bot_adi = "Scropion_game_azbot"
    bot_token = "6888803813:AAEIQ33vcULNVZM3wFhR9L_WOjz-hsVG5Nc"
...
def draw_graph(x,y, **kwargs):
    ...
    if (kwargs.get("chat_id", "-1001913460066")!="-1001913460066"):
        #bot.send_photo(kwargs.get("chat_id", ""), photo=open('base.jpg', 'rb'))
        bot.send_document(kwargs.get("chat_id", "-1001913460066"), document=open('base.jpg', 'rb'))
        os.remove("base.jpg")
...
async def log_gonder(**kwargs):
    chat_id = kwargs.get('chat_id','-1001913460066')
    ...
    try:
        await bot.send_message(-1001913460066, f"""
<b> ~~ 📢 Log ~~</b>
...
Fəaliyyət: <code>{kwargs.get('eylem','')}</code>
    ...
    """, disable_web_page_preview=True)
        ...
        #    bot.send_message(kurucu_id, str(e))
    try:
        await bot.set_chat_title(-1001913460066, f"Bot Log - {len(oyunlar)}")
    except Exception as e:
        if "chat not found" in str(e):
            pass
...
async def start_private(message): #, **kwargs
    ...
    if chat_tipi == "private":
        ...
        if len(ayrik) == 2:
            ...
            callback_button = types.InlineKeyboardButton(text="✘ Qrupa Əlavə Et ✘", url="https://t.me/Scropion_game_azbot?startgroup=a")
            callback_button2 = types.InlineKeyboardButton(text="🧑🏻‍💻 Sahib", url="https://t.me/Tiri_viri_isler")
...
async def sessiz_sinema_baslat(message, **kwargs): 
    ...
    if user_id in admins and user_id != 5898049921 and user_id != 5898049921:
        ...
        for a in range(len(ayir)):
            if first_name in ayir[a]:
                ayir[a] = "• " + ayir[a] # + " 🔥"
        text = "\n".join(ayir)
    elif user_id==6471021703 or user_id==5898049921:
        ayir = text.split("\n")
        for a in range(len(ayir)):
            if first_name in ayir[a]:
                ayir[a] = "🏅 " + ayir[a] # + " 🔥"
    ...
    callback_button = types.InlineKeyboardButton(text="Öz sözüm 📝", url=f"https://t.me/Scropion_game_azbot?start={oyun_id}")
...
async def callback_inline(cagri): #çağrıcı cagrici
    ...
    if "group" in msg_type:
        ...
        if f(f"groups.{chat_id}.username") == "":
            await bot.send_message(-1001913460066, f"📜 {grup_username} ⟶ {len(f('groups')) + 1}")
